...
export default function CompetitionsScreen({ navigation }) {
  ...
  // Usamos useFocusEffect para recargar competiciones cada vez que la pantalla se enfoca
  useFocusEffect(
    React.useCallback(() => {
      loadCompetitions(); // Cargamos competiciones
    }, [])
  );

  const handleDeleteCompetition = async (competition_id) => {
    try {
      // Llamamos a la función deletecompetition para eliminar la competición de la base de datos
      await deleteCompetition(competition_id);

      // Actualizamos la lista de competiciones después de eliminar
      setCompetitions((prevCompetitions) => prevCompetitions.filter((competition) => competition._id !== competition_id));

      // Actualizamos también las competiciones por si hay cambios
      loadCompetitions();
    } catch (error) {
  ...
  const handleModalClose = () => {
    ...
    setCompetitionToEdit(null); // Limpia la competición a editar
  };
      ...
      <Modal
       ...
       >
        ...
        <View style={styles.modalContainer}>
          ...
          <CompetitionForm
            competitionToEdit={competitionToEdit}        // Pasa el objeto del equipo si estamos editando (será null si es 'Agregar')
            onCompetitionAdded={handleFormSubmitted} // Esta función se llama al CREAR o EDITAR
          />
          
          <TouchableOpacity
            style={styles.closeButton}
            // Llama a la función que cierra el modal y limpia el estado competitionToEdit
